...
describe("Wiki", () => {
  ...
  describe("wikis regular user interaction with application", () => {
  ...
  describe("wikis premium user interaction with application", () => {
    ...
    describe("POST /wikis/create", () => {
        ...
        it("should create a new wiki with associated user ID", (done) => {
          ...
          const options = {
            url: `${base}create`,
            form: {
              title: "Example of wiki premium title",
              body: "Example of wiki premium body",
              private: false,
            }
          }
  
          request.post(options, (err, res, body) => {
            Wiki.findOne({where: {title: "Example of wiki premium title"}})
            .then((wiki) => {
              expect(wiki.title).toBe("Example of wiki premium title");
              expect(wiki.body).toContain("wiki premium body");
              expect(wiki.userId).not.toBeNull();
              done();
            })
            .catch((err) => {
        ...
        it("should create a new wiki and set post to private", (done) => {
          ...
          request.post(options, (err, res, body) => {
            Wiki.findOne({where: {title: "Private wiki title"}})
            .then((wiki) => {
              expect(wiki.title).toBe("Private wiki title");
              expect(wiki.body).toContain("Private wiki body");
        ...
        describe("GET /wikis", () => {
          ...
          it("should view wiki list but not include the private wiki", (done) => {
      
              request.get(base, (err, res, body) => {
                  expect(res.statusCode).toBe(200);
                  expect(err).toBeNull();
                  expect(body).toContain("Wikis");
                  expect(body).not.toContain("Private wiki title");
                  done();
              });
